...
func (m *MModelActivity) SetCollector(syncID string, collector pbext.ICollector, cb func(string)) {
	...
	for key, value := range m.totalRecharges {
		syncKey := fmt.Sprintf("%s.%v", m.fieldSyncIDs[1], key)
...
func (m *MModelActivity) SetLocalActivityIds(key int32, value int32) {
	localSyncKey := fmt.Sprintf("%s.%v", m.fieldSyncIDs[0], key)
	var oldValue any
...
func (m *MModelActivity) SetTotalRecharges(key int64, value *datas.MTimeLimitTotalRechargeInfo) {
	localSyncKey := fmt.Sprintf("%s.%v", m.fieldSyncIDs[1], key)
	var oldValue any
...
func (m *MSawingInfo) String() string {
	...
	strBuilder.WriteString(fmt.Sprintf("%v", m.sawingRound))
	strBuilder.WriteString(", ")
	strBuilder.WriteString("sawingStep:")
	strBuilder.WriteString(fmt.Sprintf("%v", m.sawingStep))
	strBuilder.WriteString(", ")
	strBuilder.WriteString("status:")
	strBuilder.WriteString(fmt.Sprintf("%v", m.status))
	strBuilder.WriteString("}")
	return strBuilder.String()
}
